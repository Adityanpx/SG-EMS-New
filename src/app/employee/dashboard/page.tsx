...
// Fake pulse chart data — visual only
const PULSE_DATA = [
...
export default function EmployeeDashboard() {
  ...
  useEffect(() => { if (profile) loadDashboardData() }, [profile])

  async function loadDashboardData() {
    const supabase = createClient()
    const today = new Date().toISOString().split('T')[0]

    const { data: att } = await supabase.from('attendance_records').select('*').eq('user_id', profile!.user_id).eq('date', today).single()
    setAttendance(att)

    const { data: myTasks } = await supabase.from('tasks').select('*').or(`assigned_to.eq.${profile!.user_id},assigned_to.is.null`).order('created_at', { ascending: false }).limit(5)
    setTasks(myTasks || [])

    const { data: team } = await supabase.from('profiles').select('*').eq('role', 'employee').neq('user_id', profile!.user_id).limit(4)
    setTeamMembers(team || [])

    const { data: leaves } = await supabase.from('leave_requests').select('*').eq('status', 'approved').lte('from_date', today).gte('to_date', today)
  ...
  async function handleClockIn() {
    ...
    const { error } = await supabase.from('attendance_records').insert({
      user_id: profile!.user_id, date: new Date().toISOString().split('T')[0],
      mode: selectedMode, status: 'clocked_in', clock_in_time: new Date().toISOString(),
    })
  ...
  async function handleClockOut() {
    ...
    const { error } = await supabase.from('attendance_records').update({
      status: 'clocked_out', clock_out_time: new Date().toISOString(), clock_out_summary: valid,
    }).eq('user_id', profile!.user_id).eq('date', new Date().toISOString().split('T')[0])
  ...
  return (
    <div>
      ...
      {/* ── Bento Row 1: Attendance | Team Vibe | Team Online ── */}
          ...
          {!attendance ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { console.log('Clock In clicked, opening modal...'); setShowClockIn(true) }}
      ...
      {/* ── Bento Row 2: Tasks | Out of Office + Remote Pulse ── */}
      ...
      {/* ── Clock Out Modal (Figma "Clocking Out") ─────── */}
